...
# Configure Net
net = Net(num_blocks=8, num_feat=128)
net.load_state_dict(torch.load('weight/block8_feat128.pth', map_location=torch.device('cpu')))
...
# MFFW
for j in range(1,14):
    a = to_tensor(imread(os.path.join(test_image2,str(j),'image1.tif')))[None,:,:,:]
    b = to_tensor(imread(os.path.join(test_image2,str(j),'image2.tif')))[None,:,:,:]
    with torch.no_grad():
        net.eval()
        output = net(a,b)
    theta = output.detach().squeeze()
